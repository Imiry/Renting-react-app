/*
 * @Author: sitao
 * @Date: 2021-02-20 14:00:22
 * @LastEditors: sitao
 * @LastEditTime: 2021-02-23 15:30:04
 */
...
export default class CityList extends Component {
  ...
  // 获取城市列表数据的方法
  async getCityList() {
    const {body:res} = await cityService.getCityList({level:1})
    const { cityList, cityIndex } = formatCityData(res)
    // console.log(res)

    // 获取热门城市数据
    const { body:hotres } = await cityService.getHotCityList()
    cityList['hot'] = hotres
  ...
  // 用于获取List组件中渲染行的信息
  //通过参数sartIndex获取到起始的行索引（也就是尝试列表的可视区最顶部的一行索引号）
